...
public class GraniteSemanticRouter implements SemanticRouter {
    private static final Pattern ALLOWED_CLASSIFICATION = Pattern.compile(
            "^(system-time|audio-control|os-command|current-research|general)(?:$|[\\s:;(])");
    ...
    static Capability parseClassification(String result) {
        String normalized = Objects.requireNonNull(result, "result cannot be null")
        ...
        Matcher matcher = ALLOWED_CLASSIFICATION.matcher(normalized);
        if (!matcher.find()) {
            throw new IllegalArgumentException("Unknown routing skill: " + result);
        }
        return Capability.fromValue(matcher.group(1));
    }
